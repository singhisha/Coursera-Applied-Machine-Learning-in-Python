...
def answer_one():
    ...
    return len(df[df['Class'] == 1]['V1'])/len(df)
answer_one()
...
def answer_six():    
    ...
    grid_clf_auc = GridSearchCV(lr, param_grid = grid_values, scoring = 'recall',cv = 3)
    grid_clf_auc.fit(X_train, y_train)
    y_pre = grid_clf_auc.predict(X_test)
    return np.array(grid_clf_auc.cv_results_['mean_test_score']).reshape(5,2)
