...
def answer_one():
    ...
    pos = 0
    for i in poly:
        pol_lr = PolynomialFeatures(degree=i)
        X_poly = pol_lr.fit_transform(X_train.reshape(11,1))
        linreg = LinearRegression().fit(X_poly, y_train)
        output[pos,:] = linreg.predict(pol_lr.fit_transform(X_test.reshape(100,1)))
        pos = pos + 1
    return output
answer_one()
...
def answer_two():
    ...
    for i in range(0,10):
        pol_lr = PolynomialFeatures(degree=i)
        X_poly_train = pol_lr.fit_transform(X_train.reshape(11,1))
        X_poly_test = pol_lr.fit_transform(X_test.reshape(4,1))
...
def answer_four():
    ...
    # Your code here
    pol_lr = PolynomialFeatures(degree=12)
...
def answer_five():
    ...
    for feature,importance in zip(X_train2.columns,clf.feature_importances_):
        imp_features.append((importance,feature))
    imp_features.sort(reverse=True)
    return [f[1] for f in imp_features[:5]]
answer_five()
...
def answer_six():
    ...
    training_scores, test_scores = validation_curve(SVC(kernel = 'rbf',random_state=0,C = 1), X_subset, y_subset,
                                            param_name='gamma',
...
# In[72]:
    ...
    # Your code here
    #vc_train, vc_test = answer_six()
    #gamma = np.logspace(-4,1,6)
    #plt.figure()
    #plt.plot(gamma, vc_train,'b', gamma, vc_test,'r')
